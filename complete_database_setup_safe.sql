-- Complete Database Setup for MyMusicMagic (Safe Version)
...
drop policy if exists "Users can insert their own portfolios" on public.user_portfolios;
drop policy if exists "Public can view published portfolios" on public.user_portfolios;
...
drop policy if exists "Users can view their own gallery items" on public.gallery;
...
drop policy if exists "Public can view gallery items" on public.gallery;
...
drop trigger if exists handle_user_portfolio_settings_updated_at on public.user_portfolio_settings;
create trigger handle_user_portfolio_settings_updated_at
  before update on public.user_portfolio_settings
  for each row execute procedure public.handle_updated_at();

drop trigger if exists handle_user_portfolios_updated_at on public.user_portfolios;
